...
MayaCom::MayaCom(const size_t& buffSize)
{
	...
	headOffset = (mSize / sizeof(int)) - 1;
	tailOffset = (mSize / sizeof(int)) - 2;
	loopedOffset = (mSize / sizeof(int)) - 3;
	rsOffset = (mSize / sizeof(int)) - 4;
...
{
	// Get message pointer and save message               sizeof(char) * 
...
bool MayaCom::Read(char* data, const size_t length)
{
	...
	//// Check if tail is inside available memory
...
size_t MayaCom::NextSize()
{
	...
	int dataSize = *((int*)mData + (tail / sizeof(int)));

	if (tail + dataSize > maxCharOffset)
	{
		tail = 0;
		looped = 0;
